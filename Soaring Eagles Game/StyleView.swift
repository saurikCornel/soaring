...
struct StyleView: View {
    ...
    private let cardOptions: [CardOption] = [
        ...
        CardOption(id: "rockCard", buyImage: "rockBuy", selectImage: "rockSelect", closeImage: "background2", selectedImage: "rockSelected"),
        CardOption(id: "seaCard", buyImage: "seaBuy", selectImage: "seaSelect", closeImage: "background3", selectedImage: "seaSelected")
    ]
                ...
                if isLandscape {
                    ZStack {
                        ...
                        VStack(spacing: 20) { // Оборачиваем все карточки в VStack
                            HStack(spacing: 20) { // Первый ряд с двумя карточками
                                ForEach(cardOptions.prefix(2)) { card in
                                    Button(action: {
                            ...
                            HStack(spacing: 20) { // Второй ряд с одной карточкой
                                Button(action: {
                                    if let card = cardOptions.last {
                                        handleCardAction(for: card)
                                    }
                                }) {
                                    if let card = cardOptions.last {
